...
pacman::p_load(
  ...
  ClusterR, RColorBrewer, rmdformats, ggcal)
```
...
The vignette includes example on five aspects:
...
```
TOPIC: ("hazmat transportation" OR "real-time crash prediction" OR ("vehicle routing" AND safety))
```
...
```{r fig.width=6,fig.cap='The number of motor carrier crashes by year and month in Missouri 2017'}
